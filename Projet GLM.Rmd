...
str(Data)
dim(Data)
#notre base de données comporte 47 variables avec 1180 observations.
```
...
# on voit la base de données qui est constituée des différentes variables constituant les conditions meteorologiques et caractérisant les pluies. On trouve la base de données n'a pas de données manquantes.
```


On retrouve une certaine colinearité entre les variables. Ce qui confirme qu'i ya une colineraité entre eux.

Hour et minute sont distribués de la même façon

Low.Cloud.Cover.daily.min..low.cld.lay., Medium.Cloud.Cover.daily.min..mid.cld.lay., 	High.Cloud.Cover.daily.min..high.cld.lay, Total.Cloud.Cover.daily.min..sfc sont distribués de la même façon

Wind.Speed.daily.min..900.mb., 	Wind.Gust.daily.min..sfc., Wind.Speed.daily.min..80.m.above.gnd., Wind.Speed.daily.min..10.m.above.gnd. semblent être distribués de la même manière.
...
summary(reg)
# il ya un effet significative des variables avec des étoiles. on interprete avec l'estimate:
#l'apparition des NA dans la regression.soit la matrice est de plein de rang, reflète une forte correlation lineaire entre ces variables ( et d'autres).

#En faisant une regression avec toutes les variables, on retrouve les variables suivantes comme des variables(significatives) ayant une influence sur la variable à expliquer qui est la variable pluie de demain: Mean.Sea.Level.Pressure.daily.mean..MSL., Wind.Direction.daily.mean..900.mb., Mean.Sea.Level.Pressure.daily.max..MSL., Mean.Sea.Level.Pressure.daily.min..MSL., Wind.Speed.daily.min..10.m.above.gnd., Medium.Cloud.Cover.daily.max..mid.cld.lay.

#D'autres n'étant pas significatives. On aperçoit des NA au niveau des variables Hour et Minute. Dans la suite, nous  allons faire la regression en excluant ces deux variables.
```
